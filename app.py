...
def upload():
    ...
    if file and file.filename.rsplit('.', 1)[1].lower() in {'jpg', 'jpeg', 'png', 'gif'}:
        ...
        image = image.resize((350, 85))  # Resize image to 350x85 pixels
        buffered = BytesIO()
        image.save(buffered, format='PNG')  # Save resized image to buffer
        image_data = base64.b64encode(buffered.getvalue()).decode('utf-8')  # Convert image to base64 string
